...
bool expression(int *t, std::string& value);
//＜项＞     ::= ＜因子＞{＜乘法运算符＞＜因子＞}
bool item(int *t, std::string& value);
//＜因子＞    ::= ＜标识符＞｜＜标识符＞'['＜表达式＞']'|＜标识符＞'['＜表达式＞']''['＜表达式＞']'|'('＜表达式＞')'｜＜整数＞|＜字符＞｜＜有返回值函数调用语句＞ ＜标识符＞'['＜表达式＞']'和＜标识符＞'['＜表达式＞']''['＜表达式＞']'中的＜表达式＞只能是整型，下标从0开始
bool factor(int *t, std::string& value);
...
//＜情况表＞   ::=  ＜情况子语句＞{＜情况子语句＞}
bool caseTable(int type, std::string& value, std::string& label);
//＜情况子语句＞  ::=  case＜常量＞：＜语句＞
bool caseStatement(int type, std::string& value);
//＜缺省＞   ::=  default :＜语句＞
bool defaultStatement(std::string& label);
//＜有返回值函数调用语句＞ ::= ＜标识符＞'('＜值参数表＞')'
bool callValueReturnFunction();
//＜无返回值函数调用语句＞ ::= ＜标识符＞'('＜值参数表＞')'
bool callNoValueReturnFunction();
//＜值参数表＞  ::= ＜表达式＞{,＜表达式＞}｜＜空＞
bool valueParameterTable(const std::string& funN);
//＜语句列＞   ::= ｛＜语句＞｝
bool statementList();
//＜读语句＞    ::=  scanf '('＜标识符＞{,＜标识符＞}')’
bool readStatement();
...
public:
    std::string label;
    int value;
    CASEInfo(std::string label, int value): label(label), value(value){
